...
def render_app():
    ...
    if run_analysis:
        ...
        if mode == "Find Super Stars" and len(tickers) > 1:
            ...
            logger.info(f"Spawning {max_workers} workers for {len(tickers)} tickers")
            ...
            try:
                ...
                logger.info(f"Submitted {len(future_map)} futures to executor")

                for future in as_completed(future_map, timeout=300):  # 5-min timeout per future
                    ticker = future_map[future]
                    completed += 1
                    try:
                        logger.info(f"Future completed for {ticker}")
                    ...
                    with status_placeholder.container():
                        st.caption(f"✅ Just completed: {ticker_name}")
            finally:
