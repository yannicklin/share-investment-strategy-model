...
class ASXSignalService:
    ...
    def generate_daily_signals(self):
        ...
        today_signal = Signal.for_market(self.market).filter_by(date=date.today()).first()

        if today_signal and today_signal.sent_at:
            logger.info(f"{self.market}: Signal already calculated and sent")
            return {"already_calculated": True, "already_sent": True, "signals_generated": 0}
        # Validate trading day using pandas_market_calendars
    ...
    def _process_profile(self, profile):
        ...
        for ticker in profile.stocks:
            try:
                # Add ASX suffix (.AX)
                full_ticker = (
                    ticker if ticker.endswith(self.ticker_suffix) else ticker + self.ticker_suffix
                )
                ...
                if signal_type == "BUY":
                    ...
                    if price is None:
                        logger.warning(
                            f"{self.market}: Cannot get price for {ticker}, skipping BUY signal"
                        )
                        continue
        ...
        try:
            ...
            for model_type in models_to_use:
                try:
                    ...
                    logger.warning(
                        f"{self.market}: Model {model_type} failed for {full_ticker}: {str(e)}"
                    )
                    continue
            ...
            return {"signal": consensus, "confidence": round(confidence, 2)}
