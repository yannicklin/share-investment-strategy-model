...
def dashboard():
    ...
    recent_signals = (
        Signal.query.filter(Signal.date >= seven_days_ago)
    ...
    recent_logs = JobLog.query.order_by(JobLog.created_at.desc()).limit(20).all()
    ...
    if request.method == "POST":
        try:
            ...
            profile = ConfigProfile(
                ...
                hurdle_rate=float(request.form["hurdle_rate"]) / 100.0,
                max_position_size=float(request.form["max_position_size"]),
                stop_loss=float(request.form.get("stop_loss", 0)) / 100.0
                if request.form.get("stop_loss")
                else None,
            )
...
def edit_profile(id):
    ...
    if request.method == "POST":
        try:
            ...
            profile.stop_loss = (
                float(request.form.get("stop_loss", 0)) / 100.0
                if request.form.get("stop_loss")
                else None
            )
...
def manual_trigger():
    ...
    try:
        ...
        if result["already_calculated"]:
            ...
            flash(
                f"Signal generated: {result['signal']} ({result['confidence']:.0%} confidence)",
                "success",
            )
...
def admin_whitelist():
    ...
    for admin in admins:
        admin.display_phone = (
            admin.display_phone
            if hasattr(admin, "display_phone") and admin.display_phone
            else "****"
        )
