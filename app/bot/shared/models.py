...
class Transaction(MarketIsolatedModel, db.Model):
    ...
    __table_args__ = {"comment": "Transaction audit trail with market isolation"}
...
class AdminWhitelist(db.Model):
    ...
    display_phone = db.Column(
        db.String(50), nullable=True
    )  # Formatted phone for display (+61-431-121-011)
    ...
    __table_args__ = {"comment": "Whitelisted phone numbers for admin authentication"}
...
class VerificationCode(db.Model):
    ...
    __table_args__ = {"comment": "One-time verification codes for admin authentication"}

    def __repr__(self):
        status = (
            "used"
            if self.used_at
            else ("expired" if datetime.utcnow() > self.expires_at else "active")
        )
...
class AdminSession(db.Model):
    ...
    __table_args__ = {"comment": "Active admin sessions for authenticated users"}
...
class AuthLog(db.Model):
    ...
    )  # request_code, verify_code, login, logout, session_expired
    ...
    __table_args__ = {"comment": "Audit trail for authentication events"}

    def __repr__(self):
        result = "✅" if self.success else "❌"
        return (
            f"<AuthLog {result} {self.event_type} {self.phone_number_masked} at {self.created_at}>"
        )
