...
class TestIdempotentSignalGeneration:
    ...
    def test_first_trigger_creates_signal(self, app):
        ...
        with app.app_context():
            ...
            signal = Signal.query.filter_by(date=date.today()).first()
    ...
    def test_second_trigger_skips_calculation(self, app):
        ...
        with app.app_context():
            ...
            first_signal_id = Signal.query.filter_by(date=date.today()).first().id
            ...
            signal_count = Signal.query.filter_by(date=date.today()).count()
    ...
    def test_notification_sent_only_once(self, app):
        ...
        with app.app_context():
            # Act: First trigger
            first_result = generate_daily_signals()
            signal_after_first = Signal.query.filter_by(date=date.today()).first()
            first_sent_at = signal_after_first.sent_at
            
            # Act: Second trigger
            second_result = generate_daily_signals()
            signal_after_second = Signal.query.filter_by(date=date.today()).first()
